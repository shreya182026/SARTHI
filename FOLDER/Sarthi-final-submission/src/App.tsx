...
try{
 ...
 const renderLive=()=> selected?<div className="app-page live"><div className="live-header"><div><span className="live-chip">● LIVE JOURNEY</span><h1>{to||'Your destination'}</h1><span>{selected.title} • {selected.duration} min estimate</span></div><button className="disha-mini" onClick={()=>nav('journey-health')}><Disha size={40}/></button></div><MapView center={liveCenter} path={path} markers={routeMarkers} live/><div className="live-status-row"><span><LocateFixed/> {liveCenter.lat.toFixed(4)}, {liveCenter.lng.toFixed(4)}</span><span><BatteryIcon v={battery}/>{battery}%</span><span><Wifi size={15}/> {connectivity}</span></div><div className="step-card"><div className="current-step"><span>NOW</span><b>{selected.steps[journeyStep]||selected.steps[0]}</b></div><div className="current-step next"><span>NEXT</span><b>{selected.steps[Math.min(journeyStep+1,selected.steps.length-1)]}</b></div><div className="progress-rail">{selected.steps.map((_,i)=><i className={i<=journeyStep?'done':''} key={i}/>)}</div><button className="secondary full" onClick={async()=>{
  ...
  const active=read<any>(
    'sarthi-journey-capsule',
    null
  );
 ...
 const renderSample=()=>{const sampleSteps=[Alte
